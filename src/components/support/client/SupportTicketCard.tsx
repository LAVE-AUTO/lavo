        ...
        <div className="flex min-w-0 flex-1 items-center gap-4 px-5 py-4">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-start justify-between gap-3">
              <p className="truncate text-[14px] font-bold text-[#001201] dark:text-[#FFF9EC]">{ticket.subject}</p>
          ...
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"
            className={`shrink-0 text-[#CCCCBB] transition-transform duration-200 group-hover:text-[#DDAF3B] dark:text-[#3A3A2A] ${open ? 'rotate-180' : ''}`}
            aria-hidden="true">
        ...
        <div id={`ticket-thread-${ticket.id}`} className="border-t border-[#F0EDE4] dark:border-[#1A2A14]">
          {loadingMessages ? (
            <div className="flex items-center justify-center py-8">
              <div className="h-6 w-6 animate-spin rounded-full border-[2px] border-[#DDAF3B] border-t-transparent" />
            </div>
          ) : messages.length === 0 ? (
            <div className="flex items-center justify-center py-8">
                  ...
                  <div key={msg.id} className={`flex gap-2.5 ${isAdmin ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={[
                      ...
                      isAdmin
                        ...
                        : 'rounded-tl-sm bg-[#FFF9EC] ring-1 ring-black/[0.04] dark:bg-dark-bg dark:ring-white/[0.04]',
                    ].join(' ')}>
                      {isAdmin && (
            ...
            <form onSubmit={handleSendMessage} className="border-t border-[#F0EDE4] px-5 py-3 dark:border-[#1A2A14]">
              <div className="flex gap-2">
                <textarea
                  ...
                  maxLength={5000}
                  className="flex-1 resize-none rounded-[8px] border border-[#D8D4C8] bg-[#FFF9EC] px-3 py-2 text-[13px] text-[#001201] outline-none transition-colors placeholder:text-[#BBBBAA] focus:border-[#DDAF3B] dark:border-[#001A05] dark:bg-dark-bg dark:text-[#FFF9EC] dark:placeholder:text-[#4A4A3A]"
                />
                <button
                  type="submit"
