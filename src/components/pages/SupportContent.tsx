...
function MiniAccordion({ item }: { item: FaqItem }) {
  ...
  return (
    <div className={`rounded-xl border transition-all duration-150 ${open ? 'border-gold/30' : 'border-[rgba(221,175,59,0.1)] hover:border-gold/20'}`}>
      <button type="button" onClick={() => setOpen((v) => !v)} className="w-full flex items-center justify-between px-5 py-4 text-left cursor-pointer" aria-expanded={open}>
        <span className={`text-[14px] font-semibold pr-4 ${open ? 'text-[#DDAF3B]' : 'text-[#001201] dark:text-[#d0d0c0]'}`}>{item.q}</span>
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-6 lg:px-16 pt-12 pb-20">
      ...
      <div className="max-w-2xl mb-12">
        <p className="text-[12px] font-bold tracking-[3px] uppercase text-[#DDAF3B] mb-3">{t('eyebrow')}</p>
          ...
          <Link
            ...
            className={`flex flex-col gap-3 p-6 rounded-2xl border transition-all hover:shadow-md hover:-translate-y-0.5 ${
              highlight
                ...
                : 'bg-surface border-[rgba(221,175,59,0.15)] hover:border-gold/30'
            }`}
          >
            <div className={`w-11 h-11 rounded-xl flex items-center justify-center ${highlight ? 'bg-dark-bg/20' : 'bg-gold/10 text-[#DDAF3B]'}`}>
              {icon}
            </div>
            <div>
      ...
      {/* Quick FAQ */}
      <div>
        <h2 className="text-[20px] font-black text-[#001201] dark:text-white mb-5">{t('faq_title')}</h2>
        ...
        <div className="mt-6">
          <Link href="/faq" className="inline-flex items-center gap-2 text-[14px] font-bold text-[#DDAF3B] hover:text-gold-hover transition-colors">
            {t('all_faq')}
