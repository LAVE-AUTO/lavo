...
export function AdminDisputeActionModal({ mode, maxAmount, busy, onConfirm, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Panel */}
      <div className="relative z-10 w-full max-w-[420px] animate-[fadeInUp_0.18s_ease] overflow-hidden rounded-2xl border border-[#E8E4DC] bg-white shadow-2xl dark:border-[#1E2E18] dark:bg-[#0F1A0C]">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-[#F0EDE6] px-6 py-4 dark:border-[#1A2A14]">
          <h2 className="text-[15px] font-black text-[#1A1A0A] dark:text-[#F0EDD4]">{title}</h2>
          <button type="button" onClick={onClose} disabled={busy}
            className="flex h-7 w-7 items-center justify-center rounded-lg text-[#BBBBAA] transition-colors hover:bg-[#F0EDE6] hover:text-[#555] dark:hover:bg-[#1A2A14]">
        ...
        <div className="px-6 py-5">
          {mode === 'refund_full' && (
            <p className="text-[13px] leading-relaxed text-[#666] dark:text-[#9A9A8A]">{t('modal_refund_full_hint')}</p>
          )}
