...
interface Props {
  ...
  user:      UserData | null;
  onClose:   () => void;
  onDeleted: (userId: string, permanent: boolean) => void;
}
 ...
 *
 * Soft delete: sets status to 'blocked' (yellow/orange action).
 * Hard delete: permanently removes the row. Requires typing SUPPRIMER (red action).
 */
export function AdminDeleteUserModal({ open, user, onClose, onDeleted }: Props) {
  const t = useTranslations('admin_delete_user');
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      const [ok, data] = await deleteWithApi(`/admin/users/${user.id}?permanent=${permanent}`);
      if (ok) {
        onDeleted(user.id, permanent);
        onClose();
      } else {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-[440px] animate-fade-in-up overflow-hidden rounded-2xl border border-separator/30 bg-card-surface shadow-2xl dark:border-[#1E2E18] dark:bg-dark-bg">
        ...
        <div className="flex flex-col gap-4 px-6 py-5">
          ...
          <div className="flex items-center gap-3 rounded-xl bg-[#F5F3EE] px-4 py-3 dark:bg-[#131E10]">
            ...
            <div className="min-w-0">
              ...
              <p className="truncate text-[12px] text-foreground/55 dark:text-[#B0BFB1]">{user.email}</p>
            </div>
          </div>
          ...
          {mode === 'hard' && (
            <div className="flex flex-col gap-1.5">
              <label htmlFor="delete-confirm-input" className="text-[12px] font-bold text-red-600 dark:text-red-400">
