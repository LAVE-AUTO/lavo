  ...
  if (fetchErr || !client) return (
    <div className="flex flex-col items-center gap-3 py-20 text-center">
      ...
      <Link href={'/admin/clients' as Parameters<typeof Link>[0]['href']}
        className="text-[13px] font-semibold text-[#DDAF3B] underline-offset-2 hover:underline">
        {t('back_to_list')}
      </Link>
    </div>
  );
    ...
    <div className="relative flex h-full min-h-0 flex-col overflow-hidden bg-[radial-gradient(circle_at_top,_rgba(221, 175, 59,0.12),_transparent_36%),linear-gradient(180deg,#faf8f2_0%,#f2efe7_100%)] dark:bg-[radial-gradient(circle_at_top,_rgba(221, 175, 59,0.12),_transparent_32%),linear-gradient(180deg,#001201_0%,#091009_100%)]">
      ...
      <div className="relative mx-auto flex h-full min-h-0 w-full max-w-[720px] flex-1 flex-col gap-5 overflow-y-auto scrollbar-none px-3 py-4 sm:px-4 lg:px-6 lg:py-6">
        ...
        <div className="flex items-center gap-2 text-[12px] text-foreground/55 dark:text-[#B0BFB1]">
          ...
          <span>/</span>
          <span className="font-semibold text-[#001201] dark:text-[#FFF9EC]">{displayName}</span>
        </div>
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex shrink-0 gap-2">
              <button type="button" onClick={() => setEditOpen(true)}
                className="inline-flex items-center gap-1.5 rounded-[14px] border border-[#E1DBCF] bg-white px-3 py-2 text-[13px] font-semibold text-[#5A554B] transition-colors hover:border-[#DDAF3B]/40 hover:bg-[#FCF6E5] hover:text-[#9A7A13] dark:border-[#1E2E18] dark:bg-[#0E170C] dark:text-[#A6A091]">
        ...
        <section className="rounded-[28px] border border-[#E1DBCF] bg-white/88 p-5 shadow-[0_12px_40px_rgba(26,26,10,0.06)] backdrop-blur-xl dark:border-[#1E2E18] dark:bg-[#101A0D]/90">
          <h2 className="mb-4 text-[14px] font-black text-[#001201] dark:text-[#FFF9EC]">{t('section_info')}</h2>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {[
              ...
              <div key={label} className="rounded-xl bg-[#F8F6F1] px-4 py-3 dark:bg-[#0C150B]">
                ...
                <p className="mt-1 break-all text-[13px] font-semibold text-[#001201] dark:text-[#FFF9EC]">{value}</p>
              </div>
            ))}
          </div>
        </section>
