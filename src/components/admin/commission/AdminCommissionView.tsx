    ...
    <div className="flex min-h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto bg-[#F5F5EE] p-6 dark:bg-[#0C1209]">
        <div className="grid gap-5 lg:grid-cols-[360px_1fr]">
          ...
          <div className="flex flex-col gap-4">
            <div className="overflow-hidden rounded-2xl border border-[#E8E4DC] bg-white shadow-sm dark:border-[#1E2E18] dark:bg-[#131E10]">
              <div className="border-b border-[#F0EDE6] bg-[#F9F8F5] px-5 py-3 dark:border-[#1A2A14] dark:bg-[#0E1A0C]">
                <p className="text-[11px] font-black uppercase tracking-widest text-[#AAAAAA] dark:text-[#A0A090]">{t('section_current')}</p>
              </div>
              <div className="p-5">
                <p className="mb-3 text-[11px] font-bold uppercase tracking-wider text-[#A0A090] dark:text-[#A0A090]">{t('label_rate')}</p>
                {/* Big rate input */}
                ...
                <p className="mt-2 text-[11px] leading-relaxed text-[#AAAAAA] dark:text-[#A0A090]">{t('hint_rate')}</p>

                {/* Split preview bar */}
                <div className="mt-5">
                  <p className="mb-2 text-[10px] font-black uppercase tracking-widest text-[#AAAAAA] dark:text-[#A0A090]">{t('preview_title')}</p>
                  <div className="flex h-8 overflow-hidden rounded-xl bg-[#F0EDE6] dark:bg-[#1A2A14]">
                    ...
                    <div className="flex flex-1 items-center justify-center text-[10px] font-bold text-[#888] dark:text-[#A0A090]">
          ...
          <div className="overflow-hidden rounded-2xl border border-[#E8E4DC] bg-white shadow-sm dark:border-[#1E2E18] dark:bg-[#131E10]">
            <div className="border-b border-[#F0EDE6] bg-[#F9F8F5] px-5 py-3 dark:border-[#1A2A14] dark:bg-[#0E1A0C]">
              <p className="text-[11px] font-black uppercase tracking-widest text-[#AAAAAA] dark:text-[#A0A090]">{t('section_history')}</p>
            </div>
            {historyLoading ? (
              ...
              <>
                <div className="grid grid-cols-[80px_1fr_100px] gap-4 border-b border-[#F0EDE6] px-5 py-2.5 dark:border-[#1A2A14]">
                  {[t('col_rate'), t('col_set_by'), t('col_effective')].map((h) => (
                    <span key={h} className="text-[10px] font-black uppercase tracking-widest text-[#AAAAAA] dark:text-[#A0A090]">{h}</span>
                  ))}
                </div>
                {history.map((h, i) => {
                  ...
                  return (
                    <div key={h.id}
                      ...
                      <p className="text-[11px] text-[#BBBBAA] dark:text-[#A0A090]">{formatDate(h.effective_at)}</p>
                    </div>
                  );
                })}
              </>
