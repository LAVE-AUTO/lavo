                ...
                {([1, 2, 3] as const).map((s) => (
                  <div key={s} className="flex items-center gap-1.5">
                    <div className={[
                      ...
                      step === s ? 'bg-[#C49A1E] text-[#0C1209]' : (step as number) > s ? 'bg-[#00C851]/20 text-[#00C851]' : 'bg-[#F0EDE6] text-[#999] dark:bg-[#1A2A14] dark:text-[#8A8A7A]',
                    ].join(' ')}>{(step as number) > s ? '✓' : s}</div>
                    <span className={`text-[10px] font-semibold ${step === s ? 'text-[#1A1A0A] dark:text-[#F0EDD4]' : 'text-[#999] dark:text-[#8A8A7A]'}`}>{STEP_LABELS[s]}</span>
