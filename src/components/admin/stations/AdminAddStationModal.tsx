      ...
      <div className="relative z-10 w-full max-w-[520px] animate-fade-in-up overflow-hidden rounded-2xl border border-[#E8E4DC] bg-white shadow-2xl dark:border-[#1E2E18] dark:bg-[#0F1A0C]">
                ...
                {([1, 2, 3] as const).map((s) => (
                  <div key={s} className="flex items-center gap-1.5">
                    <div className={[
                      ...
                      step === s ? 'bg-[#C49A1E] text-[#0C1209]' : (step as number) > s ? 'bg-[#00C851]/20 text-[#00C851]' : 'bg-[#F0EDE6] text-[#999] dark:bg-[#1A2A14] dark:text-[#5A5A4A]',
                    ].join(' ')}>{(step as number) > s ? '✓' : s}</div>
                    <span className={`text-[10px] font-semibold ${step === s ? 'text-[#1A1A0A] dark:text-[#F0EDD4]' : 'text-[#999] dark:text-[#5A5A4A]'}`}>{STEP_LABELS[s]}</span>
        ...
        {step === 2 && (
          <AdminAddStationInfo data={info} errors={infoErrs} busy={busy}
            onChange={setInfo} onNext={() => { if (validateInfo()) setStep(3); }} onPrev={() => setStep(1)} />
        )}
        {step === 3 && (
          <AdminAddStationDocs data={docs} errors={docsErrs} busy={busy}
            onChange={setDocs} onSubmit={handleSubmit} onPrev={() => setStep(2)} />
        )}
