...
export function AdminKpiRow({ masked = false }: { masked?: boolean }) {
  ...
  const cards: KpiCardProps[] = [
    ...
    { icon: <CommissionIcon />,   value: fmtCurrency(metrics?.total_commissions ?? '0'), label: t('kpi_platform_retained'), trendValue: String(metrics?.total_transactions ?? 0), trendUp: true, trendLabel: t('kpi_transactions_count'), sparkline: [], accentColor: GOLD, animationDelay: '60ms', masked },
