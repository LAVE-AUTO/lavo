...
export function AdminClientsList({ onAction }: Props) {
  ...
  return (
    <div className="flex flex-col gap-2">
      {clients.map((client) => {
        ...
        const badgeClass   = STATUS_STYLES[client.status] ?? 'bg-[#E0DCD0] text-[#888]';
              ...
              {isConfirming ? (
                ...
                <div className="flex gap-2">
                  ...
                  <button type="button" disabled={isBusy} onClick={() => doAction(client.id, 'unblock')}
                    className="rounded-lg border border-[#3B82F6]/30 px-3 py-1.5 text-[11px] font-bold text-[#3B82F6] hover:bg-[#3B82F6]/8 disabled:opacity-50">
                    {t('btn_unblock')}
                  </button>
                </div>
              )}
