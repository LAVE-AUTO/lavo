...
function CarIllustration({ isDark }: { isDark: boolean }) {
  const bodyFill   = isDark ? '#1E2A1A' : '#243020';
  const roofFill   = isDark ? '#243020' : '#2C3828';
  const windowFill = isDark ? '#2C3828' : '#3A4A36';
  const wheelFill  = isDark ? '#1A2116' : '#1E2A1A';
  const shadowFill = isDark ? '#0D1A0D' : '#1A2116';
  ...
  return (
    ...
    >
      {/* Car body */}
      <path d="M30 110 L50 70 Q60 55 80 55 L200 55 Q220 55 230 70 L250 110 Z" fill={bodyFill} stroke="#C49A1E" strokeWidth="2" />
      {/* Roof */}
      <path d="M80 55 L100 25 Q110 15 130 15 L160 15 Q175 15 185 25 L200 55 Z" fill={roofFill} stroke="#C49A1E" strokeWidth="1.5" />
      {/* Windows */}
      <path d="M95 52 L110 25 Q116 18 128 18 L155 18 Q166 18 172 25 L184 52 Z" fill={windowFill} stroke="#C49A1E" strokeWidth="1" opacity="0.8" />
      {/* Door line */}
      <line x1="140" y1="55" x2="140" y2="108" stroke="#C49A1E" strokeWidth="1" opacity="0.5" />
      {/* Wheels */}
      <circle cx="80"  cy="115" r="22" fill={wheelFill} stroke="#C49A1E" strokeWidth="2.5" />
      <circle cx="80"  cy="115" r="10" fill="#C49A1E" opacity="0.25" />
      <circle cx="80"  cy="115" r="4"  fill="#C49A1E" />
      <circle cx="200" cy="115" r="22" fill={wheelFill} stroke="#C49A1E" strokeWidth="2.5" />
      <circle cx="200" cy="115" r="10" fill="#C49A1E" opacity="0.25" />
      <circle cx="200" cy="115" r="4"  fill="#C49A1E" />
      {/* Ground shadow */}
      <ellipse cx="140" cy="140" rx="120" ry="8" fill={shadowFill} opacity="0.4" />
      {/* Water droplets */}
 ...
 * - Auto-cycling feature carousel with animated progress bar
 * - Locale-aware via next-intl
 * - Real logos from /public/logo/
 */
    ...
    >
      ...
      <div className="relative z-10 flex flex-col justify-between h-full p-10 xl:p-14">
        ...
        <div className="animate-fade-in">
          {isDark ? (
            <div className="w-12 h-12 rounded-full overflow-hidden border border-gold/40 bg-white shrink-0">
              <Image
                src="/logo/frame2.png"
                alt="LAVO"
                width={48}
        ...
        {/* Center: car illustration + headline */}
        ...
        <div className="flex flex-col gap-4">
          {/* Active feature card — key forces re-animation on change */}
          <div key={activeIdx} className="flex items-start gap-4 animate-fade-in-up">
            <div className="mt-0.5 w-10 h-10 rounded-xl bg-gold/15 border border-gold/30 flex items-center justify-center text-gold shrink-0">
            ...
            <div>
              ...
              <p className={`text-[13px] leading-snug ${textSecondary}`}>
          ...
          <div className="flex flex-col gap-2.5">
            {/* Animated fill bar — key resets animation on each cycle/click */}
