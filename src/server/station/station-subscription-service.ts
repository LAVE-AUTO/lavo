...
/**
 * Pulls the latest subscription state from Stripe and syncs it locally — a
 * safety net for when webhooks are delayed or not delivered (e.g. local dev or
 * a misconfigured endpoint). Resolves the subscription by its stored id, or by
 * the customer when the id has not been recorded yet (just after Checkout).
 * Returns the refreshed local row.
 */
export async function reconcileStationSubscription(stationId: string): Promise<StationSubscription | null> {
  const existing = await getStationSubscription(stationId);
  if (!existing) return null;

  let sub: Stripe.Subscription | null = null;
  if (existing.stripe_subscription_id) {
    sub = await stripe.subscriptions.retrieve(existing.stripe_subscription_id);
  } else if (existing.stripe_customer_id) {
    const list = await stripe.subscriptions.list({ customer: existing.stripe_customer_id, status: 'all', limit: 1 });
    sub = list.data[0] ?? null;
  }
  if (!sub) return existing;

  // The subscription was created with subscription_data.metadata.station_id, but
  // guard the in-memory copy so sync can always resolve the station.
  if (!sub.metadata?.station_id) {
    sub = { ...sub, metadata: { ...(sub.metadata ?? {}), station_id: stationId } } as Stripe.Subscription;
  }
  await syncSubscriptionFromStripe(sub);
  return getStationSubscription(stationId);
}
