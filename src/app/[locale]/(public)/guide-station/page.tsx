...
export async function generateMetadata({ params }: Props) {
  ...
  const t = await getTranslations({ locale, namespace: 'guide_station_page' });
  return { title: t('meta_title'), description: t('meta_desc') };
}

export default async function GuideStationPage({ params }: Props) {
  const { locale } = await params;
    ...
    <>
      ...
      <main className="min-h-screen bg-[#FFEECA] dark:bg-dark-bg transition-colors">
        <LegalContentRenderer
          contentKey="landing_guide_station"
          locale={safeLocale}
          eyebrow={t('eyebrow')}
          title={t('title')}
          updated={t('updated')}
