...
const mockListStationEntries = jest.fn();
...
jest.mock('@/server/reservations/reservation-service', () => ({
  listStationEntries: (...args: unknown[]) => mockListStationEntries(...args),
}));
...
describe('GET /api/v1/station/entries', () => {
  beforeEach(() => {
    ...
    mockListStationEntries.mockResolvedValue({
      rows: [
        {
          id: 'entry-1',
          user_id: 'user-1',
  ...
  it('returns 200 with paginated entries when authenticated as station', async () => {
    ...
    expect(mockListStationEntries).toHaveBeenCalledWith(stationId, expect.objectContaining({ page: 1 }));
  });

  it('passes query params to service', async () => {
    const res = await GET(makeRequest('status=confirmed&page=3&per_page=5'));
    expect(res.status).toBe(200);
    expect(mockListStationEntries).toHaveBeenCalledWith(
      stationId,
  ...
  it('returns 401 when auth fails', async () => {
    ...
    expect(mockListStationEntries).not.toHaveBeenCalled();
  });
